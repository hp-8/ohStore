    ...
    >
      ...
      >
        <div className="relative flex m-auto w-[95%] md:w-[90%] md:flex-wrap md:flex-row md:items-center md:mx-auto md:max-w-same">
          <div
            className={`${
              openNav ? "pb-4 border-b-2 border-orange-700 " : ""
          ...
          >
            <span className="font-sen text-2xl md:text-3xl font-semibold whitespace-nowrap dark:text-teal-50">
              <Link to={"/"}>
                <span className="text-transparent bg-clip-text bg-gradient-to-tr from-pink-700 via-lavender-300 to-yellow-300">Oh Store!</span>
              </Link>
            </span>
            ...
            <div className="order-first md:order-last flex justify-center items-center">
              <div className="mr-4 ml-8 md:flex justify-center items-center hidden">
                ...
                <div onClick={() => setShowSearch(!showSearch)}>
                  <FiSearch id="search" className="h-6 w-6 text-gray-500" />
                  <Tooltip
                    anchorId="search"
                    content="Search user"
                    place="bottom"
              ...
              {openNav ? (
                ...
                <TfiMenu
                  className="h-5 w-5 md:hidden text-gray-500"
                  onClick={() => setOpenNav(!openNav)}
                />
              )}
            </div>
            ...
            <div className="h-18 w-18">
              {theme === "dark" ? (
                <>
                  <BsSun
                    id="lightMode"
                    className={`md:h-7 md:w-7 w-6 h-6 text-gray-500 hidden md:flex`}
            ...
            {uid ? (
              <>
                <AiOutlinePoweroff
                  ...
                  className="ml-2 md:ml-4 md:h-7 md:w-7 w-6 h-6 text-gray-500 md:flex"
                  id="logoffID"
                />
              ...
              <>
                <MdLogin
                  ...
                  className="ml-2 md:ml-4 md:h-7 md:w-7 w-6 h-6 text-gray-500"
                  id="loginID"
                />
          ...
          >
            <ul className="md:m-auto flex flex-col  justify-center md:p-4 font-bold text-sm md:flex-row md:space-x-8 md:mb-0">
                ...
                if (page === currentPage) {
                    ...
                    <li key={i}>
                      <Link
                        ...
                        className="block mb-5 md:mb-0 py-2 pl-6 pr-4 rounded
                         md:bg-transparent text-orange-500 md:p-0 dark:text-white"
                      >
                        {page}
                      </Link>
                    </li>
                  ...
                  return (
                    <li key={i}>
                      <Link
                        to={`/${page.toLowerCase()}`}
                        className="block mb-5 md:mb-0 py-2 pl-6 pr-4 text-gray-700 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-orange-700 md:p-0 dark:text-gray-500 md:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700"
                      >
                        {page}
                      </Link>
                    </li>
              ...
              <li
                className="flex items-center mb-5 md:mb-0 py-2 pl-6 pr-4 text-gray-700 dark:text-gray-500 rounded md:hidden"
                onClick={handleThemeSwitch}
              >
                {theme === "dark" ? (
                  <>
                    ...
                    <BsSun
                      id="lightMode"
                      className={`md:h-7 md:w-7 w-5 h-5 text-gray-500 `}
                      // onClick={handleThemeSwitch}
                    />
                  </>
                ) : (
